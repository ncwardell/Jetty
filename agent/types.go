...
type Peer struct {
	...
	// TunnelHost is this peer's own tunnel hostname, if it has one. Lets us
	// reach THIS peer through Cloudflare when its WARP IP is unknown; the
	// cluster-wide domain would land on whichever node Cloudflare picks.
...
type peerWire struct {
	...
	// TunnelHost is the peer's own tunnel hostname. Carried here for the same
	// reason as APIKey: without it the joiner can only address peers via the
	// cluster-wide domain, which resolves to an arbitrary node.
	TunnelHost string `json:"tunnel_host,omitempty"`
}
